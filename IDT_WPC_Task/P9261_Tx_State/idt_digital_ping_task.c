/***********************************************************************************************************************
* File Name    : idt_digital_ping_task.c
...
***********************************************************************************************************************/
unsigned char IDT_WPC_Digital_Ping_Process(void);
/***********************************************************************************************************************
* Function Name: IDT_WPC_Digital_Ping_Task
* Description  : This function is IDT_WPC_Digital_Ping_Task function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void IDT_WPC_Digital_Ping_Task(void)
{
	if(IDT_WPC_State.Digital_Ping_Task_Flag==TRUE)
	{
		IDT_WPC_State.Digital_Ping_Task_Flag = FALSE;
		IDT_WPC_TASK = IDT_WPC_DIGITAL_PING_TASK;
	}
	IDT_WPC_TASK = IDT_WPC_Digital_Ping_Process();
}
/***********************************************************************************************************************
* Function Name: IDT_WPC_Digital_Ping_Process
* Description  : This function is IDT_WPC_Digital_Ping_Process function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
unsigned char IDT_WPC_Digital_Ping_Process(void)			// --- In Tx state 04 --- //
{
	...
	if(ACC_Status.Active_On_Flag==TRUE)
	{
		if(Power_Sw.Active_Flag==TRUE)
		{
			if((Door_Status.Door_Open_Flag==FALSE)&&(Trunk_Status.Trunk_Open_Flag==FALSE))
			{
				...
				else if(P9261_RegMessage.No_Response_Rx_Ept_Code_Status==TRUE)	// RxNo_Response_Rx_Ept_Code_Status 4(0x04) //
				{
				...
				else if(P9261_RegMessage.Identification_Status==TRUE)	// Tx State 05 //
				{
					P9261_RegMessage.Identification_Status = FALSE;
					IDT_WPC_State.Identification_Task_Flag = TRUE;
					IDT_WPC_NEXT_TASK = IDT_WPC_IDENTIFICATION_TASK;
				}
				else if(P9261_RegMessage.Configuration_Status==TRUE)	// Tx State 07 //
				{
