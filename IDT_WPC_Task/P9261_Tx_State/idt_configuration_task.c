/***********************************************************************************************************************
* File Name    : idt_configuration_task.c
...
***********************************************************************************************************************/
unsigned char IDT_WPC_Configuration_Process(void);
/***********************************************************************************************************************
* Function Name: IDT_WPC_Configuration_Task
* Description  : This function is IDT_WPC_Configuration_Task function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void IDT_WPC_Configuration_Task(void)
{
	if(IDT_WPC_State.Configuration_Task_Flag==TRUE)
	{
		IDT_WPC_State.Configuration_Task_Flag = FALSE;
		IDT_WPC_TASK = IDT_WPC_CONFIGURATION_TASK;
	}
	IDT_WPC_TASK = IDT_WPC_Configuration_Process();
}
/***********************************************************************************************************************
* Function Name: IDT_WPC_Configuration_Process
* Description  : This function is IDT_WPC_Configuration_Process function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
unsigned char IDT_WPC_Configuration_Process(void)			// --- In Tx state 07 --- //
{
	...
	if(ACC_Status.Active_On_Flag==TRUE)
	{
		if(Power_Sw.Active_Flag==TRUE)
		{
			if((Door_Status.Door_Open_Flag==FALSE)&&(Trunk_Status.Trunk_Open_Flag==FALSE))
			{
				...
				else if(P9261_RegMessage.No_Response_Rx_Ept_Code_Status==TRUE)	// Rx No_Response_Rx_Ept_Code_Status status 4(0x04) //
				{
				...
				else if(P9261_RegMessage.Negotiation_Status==TRUE)		// Tx State 0x13 //
				{
					P9261_RegMessage.Negotiation_Status = FALSE;
					IDT_WPC_State.Negotiation_Task_Flag = TRUE;
					IDT_WPC_NEXT_TASK = IDT_WPC_NEGOTIATION_TASK;
				}
				...
				else if(P9261_RegMessage.Negotiation_Status==TRUE)		// Tx State 13(0x0D) //
				{
					P9261_RegMessage.Negotiation_Status = FALSE;
					IDT_WPC_State.Negotiation_Task_Flag = TRUE;
					IDT_WPC_NEXT_TASK = IDT_WPC_NEGOTIATION_TASK;
